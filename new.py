...
zomato_features = getFeatures(cv2.imread('zomato.png'))
# detect features on test image

frame = cv2.imread('test2.jpeg')
# print(type(frame))
region = detectFeatures(frame, train_features)
zom_region = detectFeatures(frame, zomato_features)
print(type(zom_region))
print(type(region))
if region is not None:
    ...
    print("Dell logo found")

if zom_region is not None:
    # draw rotated bounding box
    box = cv2.boxPoints(region)
    box = np.int0(box)
    # print(type(box))
